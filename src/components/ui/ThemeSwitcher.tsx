import { useTheme, type ThemeId } from '../../context/ThemeContext'

const GRID_THEMES: ThemeId[] = ['default', 'emerald', 'midnight', 'nightfall']

const SWATCH_COLORS: Record<ThemeId, string> = {
  default: 'bg-blue-500',
  emerald: 'bg-emerald-500',
  rose: 'bg-rose-500',
  midnight: 'bg-blue-900',
  nightfall: 'bg-violet-800',
}

export function ThemeSwitcher() {
  const { theme, setTheme } = useTheme()

  return (
    <div
      className="inline-flex rounded-lg border border-secondary-200 bg-surface-raised"
      role="group"
      aria-label="Color scheme"
    >
      {GRID_THEMES.map((t) => (
        <button
          key={t}
          onClick={() => setTheme(t)}
          className={`relative w-6 h-6 rounded-md transition-all focus-visible:outline focus-visible:outline-offset-1 focus-visible:outline-primary-500 ${
            theme === t
              ? 'ring-2 ring-primary-500'
              : 'hover:scale-105 hover:opacity-90'
          }`}
          aria-label={`${t} theme`}
          aria-pressed={theme === t}
        >
          <span
            className={`absolute inset-1 rounded ${SWATCH_COLORS[t]} ${
              t === 'midnight' || t === 'nightfall' ? 'brightness-75' : ''
            }`}
          />
        </button>
      ))}
    </div>
  )
}
